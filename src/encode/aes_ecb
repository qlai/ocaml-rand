...
let aesecb encode yourkey keyfile ivfile infile outfile = 
  let checkkey akey =
  match akey with 
  ...
  | "E" -> AES.ECB.encrypt ~key:key ~iv:iv (Cstruct.of_string(padding(readfile infile)))
  | "D" -> AES.ECB.decrypt ~key:key ~iv:iv (Cstruct.of_string(padding(readfile infile)))
...
let cmd =
  ...
  let man = [
    ...
    `P "Submit via github"]
  in
  Term.(pure aesecb $ encode $ yourkey $ keyfile $ ivfile $ infile $ outfile),
